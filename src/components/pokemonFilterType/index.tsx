...
export const PokemonsFilterType: FunctionComponent<TypeProps> = ({ setPokemons }) => {
    ...
    return (
        <Div >
            <div>
                <Input {...register("name", { required: true })} type='text' className={`${errors.name && inputError}`} onKeyDown={handleKeyDown} placeholder="Search Pokemons by Type..." />
                <P>{error}</P>
                {errors?.name?.type === 'required' && <P>Type is Required</P>}
            </div>
            <Button onClick={() => handleSubmit(onSubmit)()} id='click' ><img src={pokeball} alt="pokeboll" /></Button>
        </Div>
    )
}